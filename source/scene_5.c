...
#include "level_3_tiles_bin.h"
#include "ingame_tiles_bin.h"
#include "level_3_bg_map_bin.h"
#include "level_3_col_map_bin.h"
#include "level_3_misc_map_bin.h"
...
inline void init_level_2_0()
{
	...
	dma_full_transfer_3(&tile_mem[0][0], level_3_tiles_bin, BYTES_TO_WORDS(level_3_tiles_bin_size));
	...
	dma_full_transfer_3(&map_mem[SCRBLK_IDX][0], level_3_col_map_bin, BYTES_TO_WORDS(level_3_col_map_bin_size));
	dma_full_transfer_3(&map_mem[SCRBLK_IDX + 2][0], level_3_misc_map_bin, BYTES_TO_WORDS(level_3_misc_map_bin_size));
	dma_full_transfer_3(&map_mem[SCRBLK_IDX + 4][0], level_3_bg_map_bin, BYTES_TO_WORDS(level_3_bg_map_bin_size));
	...
	print_hcenter_4bpp(INTRO_LEVEL_2, 50, textDefaultTileIdx);
	fade_in();
...
inline int run_level_2_0()
{

	uint32_t num_enemies, next_scene;

	next_scene = LEVEL_2_1;
	...
	reset_dynamic_bg_to(FIXED(256 - GBA_HEIGHT), 0);
	reset_static_bg_to(0, 0);

	add_enemy_slime(FIXED(176), FIXED(304), DIV2(FIXED(1)), DIR_LEFT, &num_enemies);
	add_enemy_slime(FIXED(224), FIXED(224), FIXED(1), DIR_LEFT, &num_enemies);
	add_enemy_slime(FIXED(112), FIXED(224), FIXED(1), DIR_LEFT, &num_enemies);
	add_enemy_slime(FIXED(32), FIXED(224), FIXED(1), DIR_LEFT, &num_enemies);
	add_enemy_fly(FIXED(32), FIXED(224), FIXED(1), DIR_LEFT, &num_enemies);
